...
public:
     ...
     * @return term weight
     */
    const double getWeight(const std::string &t, double tf, const IndexItem &d) const;
     ...
     * @return term weight
     */
    const double getQueryWeight(const std::string &t, const IndexItem &d) const;
    ...
    virtual const std::vector<QueryResult> queryByCount(const std::string, const unsigned int= 10);
...
protected:
    ...
    /**
    * a word appeared how many times in total
    */
    ...
    */
    std::map<const std::string, unsigned long> docFreMap;
...
private:
    void subThread(size_t from, size_t to);
